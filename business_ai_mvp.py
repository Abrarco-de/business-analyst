...
def process_business_data(groq_client, df_raw):
    try:
        ...
        # 3. Product Margin Logic
        df['_prod'] = df[prod_col].astype(str)
        stats = df.groupby('_prod').agg({'_rev':'sum', '_prof':'sum'})
        ...
        # RETURN DICTIONARY (Keys matched exactly to app.py)
        metrics = {
            "status": "success",
            "meta": meta,
...
def get_ai_response(mistral_client, m, query):
    ...
    ctx = f"Rev: {m.get('total_revenue')}, Profit: {m.get('total_profit')}. Margin: {m.get('margin_pct')}%"
    try:
        res = mistral_client.chat.complete(model="mistral-large-latest", messages=[{"role":"user", "content":f"{ctx}\nQuestion: {query}"}])
        return res.choices[0].message.content
    except: return "AI is currently busy. Please try again."
